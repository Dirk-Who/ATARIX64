...
/*
 * Older AtariX releases used a private SDL_KeyboardActivate() patch.
 * Official SDL does not provide that function, so control the keyboard
 * events in SDL's public event API instead.
 */
static void AtariX_EnableKeyboardEvents(bool enable)
{
	const Uint8 state = enable ? SDL_ENABLE : SDL_IGNORE;
...
)
{
	...
	if (atariKernelPathUrl && strlen(atariKernelPathUrl) < MAXPATHNAMELEN)
	{
	...
	if (atariRootfsPathUrl && strlen(atariRootfsPathUrl) < MAXPATHNAMELEN)
	{
		strcpy(Globals.s_atariRootfsPathUrl, atariRootfsPathUrl);
	}
	else
	{
...
/*********************************************************************************************************
 *
 * Toggle fullscreen mode (asynchronous function)
 *
 *********************************************************************************************************/

void EmulationRunner::ToggleFullscreen(void)
{
	DebugTrace("%s(): %d", __func__, m_sdl_window != 0);
	SDL_Event event;

	event.type = SDL_USEREVENT;
	event.user.code = TOGGLE_EMULATOR_FULLSCREEN;
	event.user.data1 = 0;
	event.user.data2 = 0;
...
//	UpdateTextureFromRect(m_sdl_texture, m_sdl_surface, NULL);
	...
	/* This legacy 32-bit host pointer is unused and cannot represent arm64 addresses. */
	pixmap->baseAddr32    = 0;		// target address, filled in by emulator
...
/*********************************************************************************************************
 *
 * Keep the host pointer hidden while AtariX owns the pointer in fullscreen
 *
 *********************************************************************************************************/
...
/*********************************************************************************************************
 *
 * Toggle borderless desktop fullscreen while preserving the Atari aspect ratio
 *
 *********************************************************************************************************/

void EmulationRunner::_ToggleFullscreen(void)
{
	if (!m_sdl_window)
		return;
...
#endif
			if (ev->type == SDL_KEYDOWN && ev->repeat == 0)
			{
				const bool macShortcut =
					ev->keysym.sym == SDLK_f &&
					(ev->keysym.mod & KMOD_GUI) != 0 &&
					(ev->keysym.mod & KMOD_CTRL) != 0;
				const bool altEnter =
					(ev->keysym.sym == SDLK_RETURN || ev->keysym.sym == SDLK_KP_ENTER) &&
					(ev->keysym.mod & KMOD_ALT) != 0;
				const bool escapeFromFullscreen =
					ev->keysym.sym == SDLK_ESCAPE &&
					m_sdl_window &&
					(SDL_GetWindowFlags(m_sdl_window) & SDL_WINDOW_FULLSCREEN_DESKTOP) != 0;
				if (macShortcut || altEnter || escapeFromFullscreen)
				{
					_ToggleFullscreen();
					break;
				}
...
void EmulationRunner::EmulatorWindowUpdate(void)
{
	if (atomic_exchange(&m_Emulator.bVideoBufChanged, 0))
	{
		...
		if (m_visible)
		{
			(void) SDL_RenderClear(m_sdl_renderer);
