...
//	int SendKeyboard(uint32_t message, bool KeyUp);
	...
	std::atomic_bool m_bEmulatorIsRunning;
...
#pragma pack(pop)
	...
	std::thread m_EmuThread;			// emulator thread
	...
	std::atomic_bool m_bBusErrorPending;
	uint32_t m_BusErrorAddress;
	char m_BusErrorAccessMode[32];

	std::atomic_bool m_bInterrupt200HzPending;
	std::atomic_bool m_bInterruptVBLPending;
	std::atomic_bool m_bInterruptMouseKeyboardPending;
	AtariPoint m_InterruptMouseWhere;
	...
	AtariEvent m_Event;
	std::atomic_bool m_bCanRun;
	std::atomic_bool m_bTerminateRequested;
	...
	AtariRecursiveMutex m_AECriticalRegion;
	...
	AtariRecursiveMutex m_ScrCriticalRegion;
