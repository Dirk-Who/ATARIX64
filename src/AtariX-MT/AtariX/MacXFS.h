...
#pragma pack(pop)
	...
	/*
	 * Native host state must not be recovered solely from pointers stored in
	 * the emulated 68k file descriptor. Keep the drive number associated with
	 * every open host descriptor on the host side instead.
	 */
	std::unordered_map<int, uint16_t> openFileDrives;
