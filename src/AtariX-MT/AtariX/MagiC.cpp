...
static inline void OS_EnterCriticalRegion(AtariRecursiveMutex &criticalRegion)
{
	criticalRegion.lock();
}

static inline void OS_ExitCriticalRegion(AtariRecursiveMutex &criticalRegion)
{
	criticalRegion.unlock();
}

static inline void OS_SetEvent(AtariEvent &event, uint32_t flags)
{
	event.Set(flags);
}

static inline void OS_WaitForEvent(AtariEvent &event, uint32_t *flags)
{
	*flags = event.Wait();
}
...
#define SetThunkCPP(a,b) { g_callbacks[a] = b; pMacXSysHdr->a.func = a; }
	...
	/* These fields were only valid for 32-bit PowerPC and are unused by the kernel. */
	pMacXSysHdr->MacSys_PPCAddr = 0;
	pMacXSysHdr->MacSys_VideoAddr = 0;
...
int CMagiC::CreateThread( void )
{
	...
	m_bTerminateRequested = false;
	m_Event.Reset();
	m_InterruptEvent.Reset();

	try
	{
		m_EmuThread = std::thread(&CMagiC::EmuThread, this);
	}
...
void CMagiC::EmuThread( void )
{
	...
	for	(;;)
	{
		...
		while (!m_bCanRun)
		{
			...
			DebugInfo("CMagiC::EmuThread() -- wait for event");
			OS_WaitForEvent(
						m_Event,
						&EventFlags);

			DebugInfo("CMagiC::EmuThread() -- event received");

			// wir prüfen, ob wir zum Beenden aufgefordert wurden

			if	((EventFlags & EMU_EVNT_TERM) || m_bTerminateRequested)
			{
...
//		DebugInfo("CMagiC::EmuThread() --- %d 68k-Zyklen", CyclesExecuted);
		...
		{
			OS_EnterCriticalRegion(m_ScrCriticalRegion);
			UpdateAtariDoubleBuffer();
			m_bScreenBufferChanged = false;
			OS_ExitCriticalRegion(m_ScrCriticalRegion);
		}
...
{
#ifdef _DEBUG_KB_CRITICAL_REGION
	DebugInfo("CMagiC::PutKeyToBuffer() --- Enter critical region m_KbCriticalRegion");
#endif
	OS_EnterCriticalRegion(m_KbCriticalRegion);
	*m_pKbWrite++ = key;
	...
	OS_ExitCriticalRegion(m_KbCriticalRegion);
#ifdef _DEBUG_KB_CRITICAL_REGION
	DebugInfo("CMagiC::PutKeyToBuffer() --- Exited critical region m_KbCriticalRegion");
#endif
}
...
void CMagiC::SendAtariFile(const char *pBuf)
{
...
#endif
	OS_EnterCriticalRegion(m_AECriticalRegion);
	...
	OS_ExitCriticalRegion(m_AECriticalRegion);
}
...
int CMagiC::SendKeyboard(uint32_t message, bool KeyUp)
{
	...
	//	DebugInfo("CMagiC::SendKeyboard() --- message == %08x, KeyUp == %d", message, (int) KeyUp);
...
#ifdef _DEBUG_KB_CRITICAL_REGION
		DebugInfo("CMagiC::SendKeyboard() --- Enter critical region m_KbCriticalRegion");
#endif
		OS_EnterCriticalRegion(m_KbCriticalRegion);
		if	(GetKbBufferFree() < 1)
		{
			OS_ExitCriticalRegion(m_KbCriticalRegion);
#ifdef _DEBUG_KB_CRITICAL_REGION
			DebugInfo("CMagiC::SendKeyboard() --- Exited critical region m_KbCriticalRegion");
#endif
...
#ifdef _DEBUG_KB_CRITICAL_REGION
		DebugInfo("CMagiC::SendKeyboardShift() --- Enter critical region m_KbCriticalRegion");
#endif
		OS_EnterCriticalRegion(m_KbCriticalRegion);
		for	(;;)
		{
			...
			if	(GetKbBufferFree() < nKeys)
			{
				OS_ExitCriticalRegion(m_KbCriticalRegion);
#ifdef _DEBUG_KB_CRITICAL_REGION
				DebugInfo("CMagiC::SendKeyboardShift() --- Exited critical region m_KbCriticalRegion");
#endif
...
int CMagiC::SendMousePosition(int x, int y)
{
	...
	if (m_bEmulatorIsRunning)
	{
...
#ifdef _DEBUG_KB_CRITICAL_REGION
		DebugInfo("CMagiC::SendMousePosition() --- Enter critical region m_KbCriticalRegion");
#endif
		OS_EnterCriticalRegion(m_KbCriticalRegion);
		...
		OS_ExitCriticalRegion(m_KbCriticalRegion);
#ifdef _DEBUG_KB_CRITICAL_REGION
		DebugInfo("CMagiC::SendMousePosition() --- Exited critical region m_KbCriticalRegion");
#endif
	}
...
int CMagiC::SendMouseButton(unsigned int NumOfButton, bool bIsDown)
{
	...
	if (m_bEmulatorIsRunning)
	{
...
#ifdef _DEBUG_KB_CRITICAL_REGION
		DebugInfo("CMagiC::SendMouseButton() --- Enter critical region m_KbCriticalRegion");
#endif
		OS_EnterCriticalRegion(m_KbCriticalRegion);
#if 0
		if	(!Globals.s_Preferences.m_KeyCodeForRightMouseButton)
		...
		OS_ExitCriticalRegion(m_KbCriticalRegion);
#ifdef _DEBUG_KB_CRITICAL_REGION
		DebugInfo("CMagiC::SendMouseButton() --- Exited critical region m_KbCriticalRegion");
#endif
	}
...
uint32_t CMagiC::AtariExec68k(uint32_t params, unsigned char *AdrOffset68k)
{
	...
	#pragma pack(pop)
	New68Context *pNew68Context =
		params ? reinterpret_cast<New68Context *>(AdrOffset68k + params) : NULL;
...
//	m68k_trace_print("68k-trace-dump-exit.txt");
	...
	// Emulator-Thread anhalten
	pTheMagiC->m_bCanRun = false;
	pTheMagiC->m_bTerminateRequested = true;
...
#pragma unused(AdrOffset68k)
...
#ifdef _DEBUG_KB_CRITICAL_REGION
	DebugInfo("CMagiC::AtariGetKeyboardOrMouseData() --- Enter critical region m_KbCriticalRegion");
#endif
	OS_EnterCriticalRegion(m_KbCriticalRegion);
	...
	if	(params)
	{
		OS_ExitCriticalRegion(m_KbCriticalRegion);
#ifdef _DEBUG_KB_CRITICAL_REGION
		DebugInfo("CMagiC::AtariGetKeyboardOrMouseData() --- Exited critical region m_KbCriticalRegion");
#endif
