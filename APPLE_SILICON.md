...
Date: 25.07.26
This branch is derived from Andreas Kromke's original AtariX and Thorsten Otto's continuation of it (see README.md for full attribution and license information).
...
- Version 0.4F supports resizable and fullscreen output with aspect-ratio
  ...
  calls now use macOS-compatible success semantics. Optional legacy Finder
  Type/Creator metadata and drive-sync requests are handled compatibly so
  Jinnee keeps copied files.
...
workflow performs an unsigned `arm64` compile-and-link check.
